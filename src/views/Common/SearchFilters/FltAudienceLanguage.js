...
export default function FltAudienceLanguage({ clearFlag, tip, icon, values, caption, itmKey, itmValue, setValues, ...rest }) {
  ...
  const setLanguage = (val) => {
    setItemValue(val);
    if (val !== '') {
      const selItm = _.findIndex(values, valItm => _.get(valItm, itmKey) === val);
      setFilterValue({title:_.get(values[selItm], itmValue), weight: 0.05 * 6});
    } else {
      setFilterValue({title:val, weight: 0.05 * 6});
    }
    setValues('language', val === '' ? null : val);
  }
  ...
  return (
    <Box className='flex-sub-wrapper'>
      <Box className='search-item-wrapper'>
        ...
        {itemValue !== '' && 
          <span className='clear' onClick={e=>setLanguage('')} style={{color:theme.palette.clrVariables.cyanLight}}>{Keyword.caption.clear}</span>
        }
      </Box>
      <Select 
        value={itemValue}
