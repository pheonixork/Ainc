...
export default function FltInfluencerEngage({ clearFlag, tip, icon, values, caption, setValues, ...rest }) {
  ...
  const setEngagements = (val) => {
    setItemValue(val);
    setValues('engagementRate', val === '' ? null : val);
  }
  ...
  return (
    <Box className='flex-sub-wrapper'>
      <Box className='search-item-wrapper'>
        ...
        {itemValue !== '' && 
          <span className='clear' onClick={e=>setEngagements('')} style={{color:theme.palette.clrVariables.cyanLight}}>{Keyword.caption.clear}</span>
        }
      </Box>
      <Select 
        value={itemValue}
