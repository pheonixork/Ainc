    ...
    <Box className={classes.audiencelikes}>
      ...
      <Box className='wrapper-grid' sx={{gridTemplateColumns: '2fr 1fr 1fr'}}>
        ...
        <Box className='box-wrapper-shadow grid-item' sx={{display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center'}}>
          ...
          <Box className='subtitle'>{`${formatter.format((data.credibility ?? 0) * 100)}%`}</Box>
          <span>いいねのアクティブ率</span>
          <RoundInfo sx={{marginLeft: '.5rem'}} caption='いいねの信憑性。著しく低い場合は要注意です。' />
        </Box>
        <Box className='box-wrapper-shadow grid-item' sx={{display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center'}}>
          <Box className='subtitle' sx={{marginTop:'1rem'}}>{`${formatter.format((data.nonFollowerLikes ?? 0) * 100)}%`}</Box>
          <span>フォロワー外からのいいね</span>
          <RoundInfo sx={{marginLeft: '.5rem'}} caption='この数値が高いほど、このインフルエンサ―の投稿は既存のフォロワーの外部に拡散されやすい傾向にあります。' />
        </Box>
      </Box>
