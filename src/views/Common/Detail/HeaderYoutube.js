...
const HeaderYoutube = ({data, stats, type}) => {
    ...
    <Box sx={{marginTop:'3rem', width: '100%'}}>
      ...
      <Box className={classes.header}>
        ...
        {type === Constants.snsInstagram && 
          <a className={classes.showFront} href={data.url} rel="noopener noreferrer" target="_blank">
      ...
      <Box className={`wrapper-grid ${classes.gridwrapper}`}>
        ...
        <Box className={`box-wrapper-shadow grid-item ${classes.showFront}`} sx={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
          ...
          <span>エンゲージメント</span>
          <RoundInfo caption='エンゲージメント率は「(いいね＋コメント)÷フォロワー」で算出しています。Instagramの平均エンゲージメント率は2%です。' sx={{marginLeft: '.5rem'}} />
        </Box>
        <Box className={`box-wrapper-shadow grid-item ${classes.showFront}`} sx={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
          <svg fill="none" height="24" width="24" xmlns="http://www.w3.org/2000/svg">
            <path d="M12 4.5C7 4.5 2.73 7.61 1 12a11.83 11.83 0 0022 0c-1.73-4.39-6-7.5-11-7.5zM12 17a5 5 0 110-10 5 5 0 010 10zm0-8a3 3 0 100 6 3 3 0 000-6z" fill="#FA8F38"></path>
          </svg>
          <Box className='subtitle'>{evaluateValue(0)}</Box>
          <span>平均ビュー</span>
          <RoundInfo caption='平均ビュー。' sx={{marginLeft: '.5rem'}} />
        </Box>
        <Box className={`box-wrapper-shadow grid-item ${classes.showFront}`} sx={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
          <svg height="24" width="24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M21.99 4A2 2 0 0020 2H4a2 2 0 00-2 2v12c0 1.1.9 2 2 2h14l4 4-.01-18zM17 14H7a1 1 0 01-1-1 1 1 0 011-1h10a1 1 0 011 1 1 1 0 01-1 1zm0-3H7a1 1 0 01-1-1 1 1 0 011-1h10a1 1 0 011 1 1 1 0 01-1 1zm0-3H7a1 1 0 01-1-1 1 1 0 011-1h10a1 1 0 011 1 1 1 0 01-1 1z" fill="#4AABE9"></path>
          </svg>
          <Box className='subtitle'>{evaluateValue(0)}</Box>
          <span>平均コメント</span>
          <RoundInfo caption='平均コメント。' sx={{marginLeft: '.5rem'}} />
        </Box>
        <Box className={`box-wrapper-shadow grid-item ${classes.showFront}`} sx={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
          <svg fill="none" height="24" width="24" xmlns="http://www.w3.org/2000/svg">
            <path d="M13.12 2.06L7.58 7.6A2 2 0 007 9.01V19c0 1.1.9 2 2 2h9c.8 0 1.52-.48 1.84-1.21l3.26-7.61A3 3 0 0020.34 8h-5.65l.95-4.58a1.49 1.49 0 00-2.52-1.36zM3 21a2 2 0 002-2v-8a2 2 0 00-2-2 2 2 0 00-2 2v8c0 1.1.9 2 2 2z" fill="#447D91"></path>
          </svg>
          <Box className='subtitle'>{evaluateValue(0)}</Box>
          <span>平均いいね数</span>
          <RoundInfo caption='平均いいね数。' sx={{marginLeft: '.5rem'}} />
        </Box>
        <Box className={`box-wrapper-shadow grid-item ${classes.showFront}`} sx={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
          <svg fill="none" height="24" width="24" xmlns="http://www.w3.org/2000/svg">
          ...
          <Box className='subtitle'>{evaluateValue(0)}</Box>
          <span>平均いや数</span>
          <RoundInfo caption='平均いや数。' sx={{marginLeft: '.5rem'}} />
        </Box>
      </Box>
    </Box>
  );
