...
const LastUpdates = ({data, stats}) => {
  ...
  return (
    <Box className='lastupdates'>
      ...
      */}
      <Box className='wrapper-grid' sx={{gridTemplateColumns: `${data.paidPostPerformance ? '1fr 1fr' : '1fr 1fr 1fr'}`}}>
        <Box className='box-wrapper-shadow grid-item' sx={{display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center'}}>
          ...
          <span className='subtitle-help'>平均いいね</span>
          <RoundInfo caption={'直近30投稿の平均いいね。'}/>
        </Box>
        <Box className='box-wrapper-shadow grid-item' sx={{display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center'}}>
          ...
          <Box className='subtitle'>{evaluateValue(data.profile.followers)}</Box>
          <span className='subtitle-help'>フォロワー</span>
          <RoundInfo caption={'合計フォロワー数。'} />
        </Box>
        <Box className='box-wrapper-shadow grid-item' sx={{display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center'}}>
          ...
          <Box className='subtitle'>{`${formatter.format(data.profile.engagementRate * 100)}%`}</Box>
          <span className='subtitle-help'>エンゲージメント</span>
          <RoundInfo caption={'エンゲージメント率は「(いいね＋コメント)÷フォロワー」で算出しています。Instagramの平均エンゲージメント率は2%です。'} />
        </Box>
        {data.paidPostPerformance && 
          <Box className='box-wrapper-shadow grid-item' sx={{display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center'}}>
            <svg fill="none" height="16" viewBox="0 0 18 16" width="18" xmlns="http://www.w3.org/2000/svg">
              <path d="M9.76458 0.366293L5.30123 4.93919C5.00314 5.2446 4.83395 5.66557 4.83395 6.10305V14.3491C4.83395 15.2571 5.55904 16 6.44527 16H13.6962C14.3407 16 14.9208 15.6038 15.1786 15.0012L17.805 8.71968C18.4818 7.08532 17.3136 5.26937 15.5814 5.26937H11.0295L11.7948 1.48888C11.8754 1.07617 11.7546 0.655194 11.4645 0.358038C10.9892 -0.120713 10.2319 -0.120713 9.76458 0.366293ZM1.61132 16C2.49754 16 3.22263 15.2571 3.22263 14.3491V7.74567C3.22263 6.83769 2.49754 6.0948 1.61132 6.0948C0.725092 6.0948 0 6.83769 0 7.74567V14.3491C0 15.2571 0.725092 16 1.61132 16Z" fill="#FA8F38"></path>
            </svg>
            <Box className='subtitle'>{`${formatter.format(data.paidPostPerformance * 100)}%`}</Box>
            <span className='subtitle-help'>PRパフォーマンス</span>
            <RoundInfo caption='PR投稿をした際のエンゲージメント率。' />
          </Box>
        }
      </Box>
    </Box>
