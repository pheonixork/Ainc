...
const strPeriod = ['2週間', '単月', '1年'];
const strPay = ['-', 'クレジットカード'];
const strStatus = ['開始', '継続', '終了'];

export default function AlertDialog({dlgState, closeDlg, saveHistory, selRow}) {
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        <Box sx={{display: 'grid', gridTemplateColumns: '1.5fr 3fr 7fr', padding: '.2rem 0'}}>
          <Box/>
          <Typography>日付</Typography>
          <LocalizationProvider dateAdapter={AdapterDateFns}>
            <MobileDatePicker
              value={historyDate}
              onChange={(newValue) => setHistoryDate(moment(newValue).format('YYYY/MM/DD'))}
        ...
        <Box sx={{display: 'grid', gridTemplateColumns: '1.5fr 3fr 7fr', padding: '.2rem 0'}}>
          ...
          <TextField value={amount} sx={{width: '200px'}} type="number" inputProps={{style:{padding:'.5rem'}}} onChange={e=>setAmount(e.target.value)}/>
        </Box>
        <Box sx={{display: 'grid', gridTemplateColumns: '1.5fr 3fr 7fr', padding: '.2rem 0'}}>
          <Box/>
          <Typography>期間</Typography>
          <Select value={period} onChange={e=>setPeriod(e.target.value)} size="small" sx={{fontSize:'14px'}}>
        ...
        <Box sx={{display: 'grid', gridTemplateColumns: '1.5fr 3fr 7fr', padding: '.2rem 0'}}>
          ...
          <Select value={status} onChange={e=>setStatus(e.target.value)} size="small" sx={{fontSize:'14px'}}>
        ...
        <Box sx={{display: 'grid', gridTemplateColumns: '1.5fr 3fr 7fr', padding: '.2rem 0'}}>
          ...
          <Select value={pay} onChange={e=>setPay(e.target.value)} size="small" sx={{fontSize:'14px'}}>
      ...
      <DialogActions>
        ...
        <Button onClick={e=>appendHistory()} autoFocus>確定</Button>
      </DialogActions>
    </Dialog>
  );
}